...
import { AtomShowFiles } from './config.js'
...
export default {
  ...
  activate(state) {
    ...
    const config = atom.config.get('atom-import-module')
    for(let projectPath of projectPaths) {
      const files = readdir(projectPath, {
        filter: (file) =>  {
          return file.endsWith('.js') || file.endsWith('.vue') || path.dirname(file).endsWith('node_modules')
        }
      })
  ...
  insert(item) {
    ...
    if (editor = atom.workspace.getActiveTextEditor()) {
      ...
      moduleName = moduleName.endsWith('.js') || moduleName.endsWith('.vue') ? moduleName.slice(0, moduleName.length -3) : moduleName
      ...
      const requireStr = `import ${moduleName} from '${requirePath}'`
